// Logo mappings for themes
export const THEME_LOGOS = {
  forest: require('../assets/images/logos/forest-theme-logo.png'),
  lightning: require('../assets/images/logos/lightning-theme-logo.png'),
  retro: require('../assets/images/logos/retro-theme-logo.png'),
  ocean: require('../assets/images/logos/ocean-theme-logo.png'),
  blossom: require('../assets/images/logos/blossom-theme-logo.png'),
  orange: require('../assets/images/logos/orange-theme-logo.png'),
};

// Theme definitions with colors and icons
export const THEMES = {
  forest: {
    name: 'Forest',
    key: 'forest',
    icon: 'leaf-outline',
    colors: {
      primary: "#4CAF50",
      textPrimary: "#2e5a2e",
      textSecondary: "#688f68",
      textDark: "#1b361b",
      placeholderText: "#767676",
      background: "#e8f5e9",
      cardBackground: "#f1f8f2",
      inputBackground: "#f4faf5",
      border: "#c8e6c9",
      white: "#ffffff",
      black: "#000000",
      success: "#388E3C",
      successLight: "#E8F5E9",
    }
  },
  lightning: {
    name: 'Lightning',
    key: 'lightning',
    icon: 'flash-outline',
    colors: {
      primary: "#8E44AD",        
      textPrimary: "#5D2E6B",    
      textSecondary: "#A569BD",  
      textDark: "#4A1B5C",       
      placeholderText: "#767676",
      background: "#F4F1F8",
      cardBackground: "#FDFCFE", 
      inputBackground: "#F9F7FC", 
      border: "#D5BDEC",         
      white: "#ffffff",
      black: "#000000",
      success: "#9B59B6",
      successLight: "#F3E5F5",
    }
  },
  retro: {
    name: 'Retro',
    key: 'retro',
    icon: 'radio-outline',
    colors: {
      primary: "#e17055",
      textPrimary: "#784e2d",
      textSecondary: "#a58e7c",
      textDark: "#50372a",
      placeholderText: "#767676",
      background: "#ede1d1",
      cardBackground: "#faf5eb",
      inputBackground: "#f7f2ea",
      border: "#e2d6c1",
      white: "#ffffff",
      black: "#000000",
      success: "#D35400",
      successLight: "#FDF2E9",
    }
  },
  ocean: {
    name: 'Ocean',
    key: 'ocean',
    icon: 'water-outline',
    colors: {
      primary: "#1976D2", 
      textPrimary: "#1a4971",
      textSecondary: "#6d93b8", 
      textDark: "#0d2b43", 
      placeholderText: "#767676",
      background: "#e3f2fd", 
      cardBackground: "#f5f9ff",
      inputBackground: "#f0f8ff", 
      border: "#bbdefb",
      white: "#ffffff",
      black: "#000000",
      success: "#0277BD",
      successLight: "#E1F5FE",
    }
  },
  blossom: {
    name: 'Blossom',
    key: 'blossom',
    icon: 'flower-outline',
    colors: {
      primary: "#EC407A", 
      textPrimary: "#7d2150", 
      textSecondary: "#b06a8f", 
      textDark: "#5a1836", 
      placeholderText: "#767676",
      background: "#fce4ec", 
      cardBackground: "#fff5f8", 
      inputBackground: "#fef8fa", 
      border: "#f8bbd0",
      white: "#ffffff",
      black: "#000000",
      success: "#AD1457",
      successLight: "#FCE4EC",
    }
  },
  orange: {
    name: 'Orange',
    key: 'orange',
    icon: 'sunny-outline',
    colors: {
      primary: "#FF8A65",    
      textPrimary: "#BF4226",   
      textSecondary: "#FF7043", 
      textDark: "#8D2F1A",       
      placeholderText: "#767676",
      background: "#FFF3E0",     
      cardBackground: "#FFFBF7", 
      inputBackground: "#FFF8F1", 
      border: "#FFCC9C",         
      white: "#ffffff",
      black: "#000000",
      success: "#F57C00",
      successLight: "#FFF3E0",
    }
  }
};

// Get theme by key
export const getTheme = (themeKey) => {
  return THEMES[themeKey] || THEMES.lightning; // Default to lightning theme
};

// Get all available themes as array
export const getAllThemes = () => {
  return Object.values(THEMES);
};

// Get theme colors
export const getThemeColors = (themeKey) => {
  const theme = getTheme(themeKey);
  return theme.colors;
};

// Get theme name
export const getThemeName = (themeKey) => {
  const theme = getTheme(themeKey);
  return theme.name;
};

// Get theme icon
export const getThemeIcon = (themeKey) => {
  const theme = getTheme(themeKey);
  return theme.icon;
};

export default THEMES;